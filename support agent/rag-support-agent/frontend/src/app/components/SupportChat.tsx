...
interface Message {
  ...
  query?: string; // store the triggering user query for feedback
}
...
export function SupportChat() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setError(controller.signal.aborted ? "Request timed out (30s). Is Ollama running?" : msg);
    } finally {
      clearTimeout(timer);
      setLoading(false);
    }
  ...
  const getConfidenceIcon = (level: string) => {
    ...
    if (level === "low") return <AlertCircle color={color} size={14} />;
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "rgba(160,180,230,0.6)" }}>
      ...
      >
              ...
              >
                <div
                  style={{
                    fontSize: 11,
                    fontWeight: 700,
                    color: msg.role === "user" ? "rgba(0,212,255,0.8)" : "rgba(160,180,230,0.6)",
                    marginBottom: 6,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8, marginLeft: 4 }}>
                {msg.confidence && (
                  <div
                    style={{
                      ...
                      gap: 6,
                      fontSize: 12,
                      color: CONFIDENCE_COLORS[msg.confidence.level] || "#888",
                      background: "rgba(255,255,255,0.03)",
                      border: `1px solid ${CONFIDENCE_COLORS[msg.confidence.level] || "#888"}22`,
                      borderRadius: 8,
                      padding: "4px 10px",
                      width: "fit-content",
                    }}
                  >
                    ...
                    <HelpCircle
                      size={12}
                      title={msg.confidence.explanation}
                ...
                {!msg.feedbackSubmitted && feedbackOpen !== idx && (
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <span style={{ fontSize: 11, color: "rgba(160,180,230,0.4)" }}>Was this helpful?</span>
                    <button
                      onClick={() => { setFeedbackOpen(idx); setFeedbackRating(5); }}
                      style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(0,255,136,0.6)", padding: 4 }}
                      title="Yes, helpful"
                    >
                      <ThumbsUp size={14} />
                    </button>
                    <button
                      onClick={() => { setFeedbackOpen(idx); setFeedbackRating(2); }}
                      style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,108,108,0.6)", padding: 4 }}
                      title="Not helpful"
                    >
                      <ThumbsDown size={14} />
                    </button>
                ...
                {feedbackOpen === idx && (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: "rgba(160,180,230,0.8)" }}>Rate this answer</span>
                      <button
                        onClick={() => { setFeedbackOpen(null); setFeedbackRating(0); setFeedbackComment(""); }}
                        style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(160,180,230,0.4)" }}
                      >
                        <X size={14} />
                      </button>
                    </div>
                      ...
                      {[1, 2, 3, 4, 5].map((r) => (
                        <button
                          ...
                          style={{
                            ...
                            height: 32,
                            borderRadius: 8,
                            border: `1px solid ${feedbackRating >= r ? "rgba(0,212,255,0.5)" : "rgba(255,255,255,0.1)"}`,
                            background: feedbackRating >= r ? "rgba(0,212,255,0.1)" : "transparent",
                            color: feedbackRating >= r ? "#00D4FF" : "rgba(160,180,230,0.4)",
                            cursor: "pointer",
                            fontSize: 13,
                            fontWeight: 700,
                          }}
                    ...
                    />
                    <button
                      onClick={() => handleFeedbackSubmit(idx)}
        ...
        {clarification && (
          ...
          >
            ...
            <div style={{ color: "#e8f0ff", fontSize: 14, lineHeight: 1.6, marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {clarification.options.map((opt, i) => (
                <button
                  ...
                  style={{
                    padding: "10px 14px",
                    background: selectedClarification === opt ? "rgba(0,212,255,0.15)" : "rgba(255,255,255,0.04)",
        ...
        {loading && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, color: "rgba(160,180,230,0.5)", fontSize: 13 }}>
